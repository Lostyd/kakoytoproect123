    ...
    class Music {
      constructor(client, options) {
        ...
        this.play = {
          ...
          run: "playFunction",
          alt: (options && options.play && options.play.alt) || ['p',"воспроизведи","в"],
          help: (options && options.play && options.play.help) || "Поставить в очередь песню/воспроизвести плейлис по url или названию",
        ...
        this.help = {
          ...
          run: "helpFunction",
          alt: (options && options.help && options.help.alt) || ['h','помощь',"п","помоги"],
          help: (options && options.help && options.help.help) || "Помощь с командами",
        ...
        this.pause = {
          ...
          run: "pauseFunction",
          alt: (options && options.pause && options.pause.alt) || ['stop','пауза'],
          help: (options && options.pause && options.pause.help) || "Поставить на паузу",
        ...
        this.resume = {
          ...
          run: "resumeFunction",
          alt: (options && options.resume && options.resume.alt) || ['продолжи'],
          help: (options && options.resume && options.resume.help) || "Продолжить воспроизведение",
        ...
        this.leave = {
          ...
          run: "leaveFunction",
          alt: (options && options.leave && options.leave.alt) || ['l','выйди',"ливни"],
          help: (options && options.leave && options.leave.help) || "бот выйдет из голосового канала",
        ...
        this.queue = {
          ...
          run: "queueFunction",
          alt: (options && options.queue && options.queue.alt) || ['плейлист',"покажи плейлист",'q'],
          help: (options && options.queue && options.queue.help) || "Показать плейлист",
        ...
        this.np = {
          ...
          run: "npFunction",
          alt: (options && options.np && options.np.alt) || ['что играет?',"текущий трек","nowplay"],
          help: (options && options.np && options.np.help) || "Показать текущую песню",
        ...
        this.loop = {
          ...
          run: "loopFunction",
          alt: (options && options.loop && options.loop.alt) || ["покругу", "зацикли"],
          help: (options && options.loop && options.loop.help) || "зациклить плейлист",
        ...
        this.search = {
          ...
          run: "searchFunction",
          alt: (options && options.search && options.search.alt) || ['s10',"найди","поиск"],
          help: (options && options.search && options.search.help) || "Найти 10 видео по названию",
        ...
        this.clearqueue = {
          ...
          run: "clearFunction",
          alt: (options && options.clear && options.clear.alt) || ['cq',"очистить","удали плейлист"],
          help: (options && options.clear && options.clear.help) || "Очистить плейлист",
        ...
        this.volume = {
          ...
          run: "volumeFunction",
          alt: (options && options.volume && options.volume.alt) || ['vol','громкость','г'],
          help: (options && options.volume && options.volume.help) || "Изменить громкость (от 1 до 200)",
        ...
        this.remove = {
          ...
          run: "removeFunction",
          alt: (options && options.remove && options.remove.alt) || ['r',"удалить", "удали"],
          help: (options && options.remove && options.remove.help) || "Удалить песню из плейлиста (нужно указать позицию трека)",
        ...
        this.skip = {
          ...
          run: "skipFunction",
          alt: (options && options.skip && options.skip.alt) || ['s',"пропустить", "пропусти"],
          help: (options && options.skip && options.skip.help) || "Пропустить песню",
        ...
        this.anyoneCanSkip = (options && typeof options.anyoneCanSkip !== 'undefined' ? options && options.anyoneCanSkip : true);
        this.anyoneCanLeave = (options && typeof options.anyoneCanLeave !== 'undefined' ? options && options.anyoneCanLeave : true);
        this.djRole = (options && options.djRole) || "DJ";
        this.anyoneCanPause = (options && typeof options.anyoneCanPause !== 'undefined' ? options && options.anyoneCanPause : true);
        this.anyoneCanAdjust = (options && typeof options.anyoneCanAdjust !== 'undefined' ? options && options.anyoneCanAdjust : true);
        this.youtubeKey = (options && options.youtubeKey);
        this.botPrefix = (options && options.botPrefix) || "()";
        this.defVolume = (options && options.defVolume) || 10;
 ...
 */     if (musicbot.cooldown.exclude.includes("skip")) console.warn(`[MUSIC] Excluding SKIP CMD from cooldowns can cause issues.`);
        ...
        if (musicbot.commands.has(command)) {
          ...
          if (tCmd.enabled) {
            if (!musicbot.cooldown.enabled == true && !musicbot.cooldown.exclude.includes(tCmd.masked)) {
              if (musicbot.recentTalk.has(msg.author.id)) {
                if (musicbot.cooldown.enabled == true && !musicbot.cooldown.exclude.includes(tCmd.masked)) return msg.channel.send(musicbot.note("fail", "Подождите немного и попытайтесь использовать команду снова"));
              }
          ...
          if (aCmd.enabled) {
            if (!musicbot.cooldown.enabled == true && !musicbot.cooldown.exclude.includes(aCmd.masked)) {
              if (musicbot.recentTalk.has(msg.author.id)) {
                if (musicbot.cooldown.enabled == true && !musicbot.cooldown.exclude.includes(aCmd.masked)) return msg.channel.send(musicbot.note("fail", "Подождите немного и попытайтесь использовать команду снова"));
              }
              musicbot.recentTalk.add(msg.author.id);
              setTimeout(() => { musicbot.recentTalk.delete(msg.author.id) }, musicbot.cooldown.timer);
            }
    ...
    musicbot.playFunction = (msg, suffix, args) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Вы не находитесь в голосовом канале`));
      if (!suffix) return msg.channel.send(musicbot.note('fail', 'Видео не указано!'));
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        msg.channel.send(musicbot.note("search", `Ищем видио~`));
        ...
        ytpl(playid, function(err, playlist) {
          if(err) return msg.channel.send(musicbot.note('fail', `Что то пошло не так при попытке получить данные!`));
          if (playlist.items.length <= 0) return msg.channel.send(musicbot.note('fail', `Не удалось получить видио`));
          if (playlist.total_items >= 1000) return msg.channel.send(musicbot.note('fail', `Больше нельзя добовлять видио в лейлист. Максимальное количество песен - 100`));
          var index = 0;
          var ran = 0;
          ...
          playlist.items.forEach(video => {
            ...
            if (ran >= playlist.items.length) {
              if (index == 0) msg.channel.send(musicbot.note('fail', `Неудалось получит ниодного видио из плейлиста!`))
              else if (index == 1) msg.channel.send(musicbot.note('note', `В очереди одна песня.`));
              else if (index > 1 && index < 5) msg.channel.send(musicbot.note('note', `В очереди ${index} песни.`))
              else if(index > 4)msg.channel.send(musicbot.note('note', `В очереди ${index} песен.`));
            }
          });
        });
      } else {
        msg.channel.send(musicbot.note("search", `\`Ищем: ${searchstring}\`~`));
        new Promise(async (resolve, reject) => {
          let result = await musicbot.searcher.search(searchstring, { type: 'video' });
          resolve(result.first)
        }).then((res) => {
          if (!res) return msg.channel.send(musicbot.note("fail", "Что-то пошло не так. Попробуйте снова!"));
          ...
          if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
            ...
            try {
              embed.setAuthor('Добавил в плейлист', client.user.avatarURL);
              ...
              embed.addField("Добавлена", res.queuedOn, musicbot.inlineEmbeds);
              ...
              if (musicbot.requesterName && resMem) embed.setFooter(`Добавил ${client.users.get(res.requester).username}`, res.requesterAvatarURL);
            ...
            try {
              ...
              .replace(/~/g, '\\~')
              .replace(/`/g, '\\`');
              msg.channel.send(`Сейчас играет: **${songTitle}**\nДобавил: ${client.users.get(res.requester).username}\nДобавлена: ${res.queuedOn}`)
            } catch (e) {
    ...
    musicbot.helpFunction = (msg, suffix, args) => {
      ...
      if (!suffix) {
        if (msg.channel.permissionsFor(msg.guild.me)
          .has('EMBED_LINKS')) {
          const embed = new Discord.RichEmbed();
          embed.setAuthor("Команды", msg.author.displayAvatarURL);
          embed.setDescription(`Используй \`${musicbot.botPrefix}${musicbot.help.name} [название команды]\` что бы узнать как ей пользоваться. Пользователи с ролью \`${musicbot.djRole}\` могут использовать любую команду`);
          ...
          var cmdmsg = `= Музыкальные команды =\nИспользуй ${musicbot.botPrefix}${musicbot.help.name} [название команды] что бы узнать как она работает. Пользователи с ролью \`${musicbot.djRole}\` могут использовать все команды.\n`;
          let index = 0;
          let max = musicbot.commandsArray.length;
          for (var i = 0; i < musicbot.commandsArray.length; i++) {
        ...
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          if (command.exclude) return msg.channel.send(musicbot.note('fail', `${suffix} несуществующая команда!`));
          embed.setAuthor(command.name, msg.client.user.avatarURL);
          embed.setDescription(command.help);
          if (command.alt.length > 0) embed.addField(`Альтернативные имена`, command.alt.join(", "), musicbot.inlineEmbeds);
        ...
        msg.channel.send(musicbot.note('fail', `${suffix} несуществующая команда!`));
      };
    };

    musicbot.skipFunction = (msg, suffix, args) => {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Ничего не воспроизводится.'));
      const queue = musicbot.getQueue(msg.guild.id);
      if (!musicbot.canSkip(msg.member, queue)) return msg.channel.send(musicbot.note('fail', `Вы не можете использовать эту команду.`));

      if (musicbot.queues.get(msg.guild.id).loop == "song") return msg.channel.send(musicbot.note("fail", "Невозможно пропустить, пока в цикле одно видео"));
      ...
      if (!dispatcher || dispatcher === null) {
        ...
        return msg.channel.send(musicbot.note("fail", "Что-то пошло не так"));
      };
      if (voiceConnection.paused) dispatcher.end();
      dispatcher.end();
      msg.channel.send(musicbot.note("note", "Песня пропущенна"));
    };

    musicbot.pauseFunction = (msg, suffix, args) => {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Ничего не воспроизвоится'));
      if (!musicbot.isAdmin(msg.member) && !musicbot.anyoneCanPause) return msg.channel.send(musicbot.note('fail', 'Видео поставленно на паузу'));

      const dispatcher = voiceConnection.player.dispatcher;
      if (dispatcher.paused) return msg.channel.send(musicbot.note(`fail`, `Уже на паузе!`))
      else dispatcher.pause();
      msg.channel.send(musicbot.note('note', 'Видео поставленно на паузу.'));
    };

    musicbot.resumeFunction = (msg, suffix, args) => {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Ничего не воспроизводится'));
      if (!musicbot.isAdmin(msg.member) && !musicbot.anyoneCanPause) return msg.channel.send(musicbot.note('fail', `Вы не можете использовать это команду`));

      const dispatcher = voiceConnection.player.dispatcher;
      if (!dispatcher.paused) return msg.channel.send(musicbot.note('fail', `Видео уже воспроизводится`))
      else dispatcher.resume();
      msg.channel.send(musicbot.note('note', 'Видео снято с паузы'));
    };

    musicbot.leaveFunction = (msg, suffix) => {
      if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Я не в голосовом канале'));
        musicbot.emptyQueue(msg.guild.id);

        if (!voiceConnection.player.dispatcher) return;
        voiceConnection.player.dispatcher.end();
        voiceConnection.disconnect();
        msg.channel.send(musicbot.note('note', 'Спасибо, я пожалуй пойду'));
      } else {
        const chance = Math.floor((Math.random() * 100) + 1);
        if (chance <= 10) return msg.channel.send(musicbot.note('fail', `Боюсь, я не могу позволить тебе сделать это, ${msg.author.username}.`))
        else return msg.channel.send(musicbot.note('fail', 'Извини, у тебя недостаточно прав'));
      }
    }
    ...
    musicbot.npFunction = (msg, suffix, args) => {
      ...
      if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note('note', 'Плейлист пуст'));

      if (msg.channel.permissionsFor(msg.guild.me)
        .has('EMBED_LINKS')) {
        const embed = new Discord.RichEmbed();
        try {
          embed.setAuthor('Сейчас играет', client.user.avatarURL);
          ...
          embed.addField("Добавлено", queue.last.queuedOn, musicbot.inlineEmbeds);
          ...
          if (musicbot.requesterName && resMem) embed.setFooter(`Добавил ${client.users.get(queue.last.requester).username}`, queue.last.requesterAvatarURL);
          if (musicbot.requesterName && !resMem) embed.setFooter(`Добавил \`UnknownUser (ID: ${queue.last.requester})\``, queue.last.requesterAvatarURL);
          msg.channel.send({
            embed
          });
        } catch (e) {
        ...
        try {
          var songTitle = queue.last.title.replace(/\\/g, '\\\\')
            ...
            .replace(/~/g, '\\~')
            .replace(/`/g, '\\`');
          msg.channel.send(`Сейчас играет: **${songTitle}**\nДобавил: ${client.users.get(queue.last.requester).username}\nДобавлена: ${queue.last.queuedOn}`)
        } catch (e) {
          console.error(`[${msg.guild.name}] [npCmd] ` + e.stack);
        };
      }
    ...
    musicbot.queueFunction = (msg, suffix, args) => {
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note("fail", "Не удалось найти очередь для этого сервера."));
      ...
      if (suffix) {
        ...
        if (!video) return msg.channel.send(musicbot.note("fail", "Я не смог найти это видео"));
        const embed = new Discord.RichEmbed()
        .setAuthor('Песня из очереди', client.user.avatarURL)
        ...
        .addField("Добавлена", video.queuedOn, musicbot.inlineEmbeds)
        .addField("Позиция", video.position + 1, musicbot.inlineEmbeds);
        ...
        if (musicbot.requesterName && resMem) embed.setFooter(`Добавил ${client.users.get(video.requester).username}`, video.requesterAvatarURL);
        if (musicbot.requesterName && !resMem) embed.setFooter(`Добавил \`UnknownUser (ID: ${video.requester})\``, video.requesterAvatarURL);
        ...
        if (queue.songs.length > 11) {
          ...
          const embed = new Discord.RichEmbed();
          embed.setAuthor('Плейлист', client.user.avatarURL);
          embed.setColor(musicbot.embedColor);
          ...
          msg.channel.send(embed).then(m => {
            m.react('⏪').then( r => {
              ...
              forwardsFilter.on('collect', r => {
                ...
                page++;
                embed.setDescription(pages[page - 1]);
                embed.setFooter(`Страница ${page} из ${pages.length}`, msg.author.displayAvatarURL);
                m.edit(embed);
              })
          ...
          embed.setFooter(`Страница 1 из 1`, msg.author.displayAvatarURL);
    ...
    musicbot.searchFunction = (msg, suffix, args) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Вы не в голосовом канале~`));
      if (!suffix) return msg.channel.send(musicbot.note('fail', 'Видео не указано!'));
      const queue = musicbot.getQueue(msg.guild.id);
      if (queue.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('fail', 'Максимальный размер очереди достигнут!'));
      ...
      msg.channel.send(musicbot.note('search', `Ищем: \`${searchstring}\``))
        .then(response => {
          musicbot.searcher.search(searchstring, {
            ...
            .then(searchResult => {
              if (!searchResult.totalResults || searchResult.totalResults === 0) return response.edit(musicbot.note('fail', 'Не удалось получить результаты поиска'));

              const startTheFun = async (videos, max) => {
                if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
                  const embed = new Discord.RichEmbed();
                  embed.setTitle(`Выберите видео`);
                  embed.setColor(musicbot.embedColor);
                  var index = 0;
                  ...
                  embed.setFooter(`Найдено для: ${msg.author.username}`, msg.author.displayAvatarURL);
                  msg.channel.send({
                    embed
                  })
                  .then(firstMsg => {
                    ...
                    .then(collected => {
                      ...
                      if (mcon === "cancel") return firstMsg.edit(musicbot.note('note', 'Поиск отменен'));
                      ...
                      if (song_number >= 0) {
                        ...
                        embed.setAuthor('Добалено в очередь', client.user.avatarURL);
                        ...
                        embed.addField("Позиция", videos[song_number].queuedOn, musicbot.inlineEmbeds);
                  ...
                  msg.channel.send(`\`\`\`\n= Выбери нужное видео =\n${vids}\n\n= Напиши Cancel что бы отменить поиск =`).then(firstMsg => {
                    var filter = null;
                    if (max === 0) {
                      filter = m => m.author.id === msg.author.id &&
                      m.content.includes('1') ||
                    ...
                    .then(collected => {
                      ...
                      if (song_number >= 0) {
                        ...
                        videos[song_number].position = queue.songs.length ? queue.songs.length : 0;
                        var embed = new Discord.RichEmbed();
                        embed.setAuthor('Добавлена в очередь', client.user.avatarURL);
                        var songTitle = videos[song_number].title.replace(/\\/g, '\\\\')
                        .replace(/\`/g, '\\`')
                        .replace(/\*/g, '\\*')
                        .replace(/_/g, '\\_')
                        ...
                        if (musicbot.bigPicture) embed.setImage(`https://img.youtube.com/vi/${videos[song_number].id}/maxresdefault.jpg`);
                        const resMem = client.users.get(videos[song_number].requester);
                        if (musicbot.requesterName && resMem) embed.setFooter(`Добавил ${client.users.get(videos[song_number].requester).username}`, videos[song_number].requesterAvatarURL);
                        if (musicbot.requesterName && !resMem) embed.setFooter(`добавил \`UnknownUser (ID: ${videos[song_number].requester})\``, videos[song_number].requesterAvatarURL);
                        msg.channel.send({
                          embed
                        }).then(() => {
                          queue.songs.push(videos[song_number]);
    ...
    musicbot.volumeFunction = (msg, suffix, args) => {
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Ничего не воспроизводится'));
      if (!musicbot.canAdjust(msg.member, musicbot.queues.get(msg.guild.id))) return msg.channel.send(musicbot.note('fail', `Вы не можете использовать эту команду`));
      const dispatcher = voiceConnection.player.dispatcher;

      if (!suffix) return msg.channel.send(musicbot.note('fail', 'Громкость не указана'));
      suffix = parseInt(suffix);
      if (suffix > 200 || suffix <= 0) return msg.channel.send(musicbot.note('fail', 'Громкость указана вне доступного диапазона. Укажите громкость от 1 до 200'));

      dispatcher.setVolume((suffix / 100));
      musicbot.queues.get(msg.guild.id).volume = suffix;
      msg.channel.send(musicbot.note('note', `Громкость изменена ${suffix}%.`));
    };

    musicbot.clearFunction = (msg, suffix, args) => {
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note("fail", "Не найден плейлист для этого сервера"));
      if (!musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", `Вы не можете использовать эту команду`));
      musicbot.emptyQueue(msg.guild.id).then(res => {
        msg.channel.send(musicbot.note("note", "Плейлист очищен"));
        const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
        ...
        return msg.channel.send(musicbot.note("fail", "Что то пошло не так при очисте плейлиста"));
      })
    };

    musicbot.removeFunction = (msg, suffix, args) => {
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note('fail', `Не найден плейлист для этого сервера!`));
      if (!suffix)  return msg.channel.send(musicbot.note("fail", "Нет видео на указанной позиции"));
      if (parseInt(suffix - 1) == 0) return msg.channel.send(musicbot.note("fail", "Вы не можете удалить текущее видео"));
      let test = musicbot.queues.get(msg.guild.id).songs.find(x => x.position == parseInt(suffix - 1));
      if (test) {
        if (test.requester !== msg.author.id && !musicbot.isAdmin(msg.member)) return msg.channel.send(musicbot.note("fail", "Вы не можете удалить этот элемент"));
        ...
        musicbot.updatePositions(newq, msg.guild.id).then(res => {
          ...
          msg.channel.send(musicbot.note("note", `Удалено:  \`${test.title.replace(/`/g, "'")}\``));
        })
      } else {
        msg.channel.send(musicbot.note("fail", "Не удалось найти видео или что то пошло не так"));
      }
    };

    musicbot.loopFunction = (msg, suffix, args) => {
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note('fail', `Плейлист для этого сервера не найден!`));
      ...
      new Promise((resolve, reject) => {
          ...
          if (voiceConnection === null) {
            if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
              ...
              msg.channel.send(musicbot.note('fail', 'У меня недостаточно прав, что бы присоедениться к вашему голосовому каналу!'))
              reject();
            } else {
          ...
          if (!video) {
            ...
            if (!video) {
              msg.channel.send(musicbot.note('note', 'Воспроизведение завершено!'));
              musicbot.emptyQueue(msg.guild.id);
              const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
              if (voiceConnection !== null) return voiceConnection.disconnect();
            }
          ...
          if (musicbot.messageNewSong == true && queue.last && musicbot.queues.get(msg.guild.id).loop !== "song") {
            ...
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              const embed = new Discord.RichEmbed()
              .setTitle("Сейчас играет", `${req !== null ? req.displayAvatarURL : null}`)
              ...
              .setColor(musicbot.embedColor)
              .setFooter(`Добавил ${req !== null ? req.username : "Unknwon User"}`, `${req !== null ? req.displayAvatarURL : null}`);
              msg.channel.send({embed});
            } else {
          ...
          try {
            ...
            connection.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `Что то пошло не так при подключении. Попытаюсь обновить плейлист...`));
              musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
            });

            dispatcher.on('error', (error) => {
              console.error(error);
              if (msg && msg.channel) msg.channel.send(musicbot.note('fail', `Что то пошло не так при воспроизведении. Попытаюсь обновить плейлист`));
              musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
            });
                ...
                if (musicbot.queues.get(msg.guild.id).songs.length > 0) {
                  if (loop == "none" || loop == null) {
                    ...
                    musicbot.updatePositions(musicbot.queues.get(msg.guild.id).songs, msg.guild.id).then(res => {
                      musicbot.queues.get(msg.guild.id).songs = res;
                      musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                    }).catch(() => { console.error(new Error("Что то пошло не так при смене видео")); });
                  } else if (loop == "queue" || loop == "song") {
                    musicbot.executeQueue(msg, musicbot.queues.get(msg.guild.id));
                  };
                } else if (musicbot.queues.get(msg.guild.id).songs.length <= 0) {
                  if (msg && msg.channel) msg.channel.send(musicbot.note('note', 'Воспроизведение завершено.'));
                  musicbot.queues.set(msg.guild.id, {songs: [], last: null, loop: "none", id: msg.guild.id, volume: musicbot.defVolume});
                  if (musicbot.musicPresence) musicbot.updatePresence(musicbot.queues.get(msg.guild.id), msg.client, musicbot.clearPresence).catch((res) => { console.warn(`[MUSIC] Problem updating MusicPresence`); });
                  const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
                  if (voiceConnection !== null) return voiceConnection.disconnect();
